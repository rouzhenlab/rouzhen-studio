...
async function handleUpload(request, env) {
  ...
  if (thumbnail && typeof thumbnail !== "string" && thumbnail.size > 0) {
    // v0.3.17: 统一缩略图路径 assets/thumbnails/{asset_id}.webp
    ...
    try {
      ...
      thumbnailUrl = `/api/thumb/${thumbnailKey}`;
    } catch (err) {
      // 缩略图写入失败不影响原图上传
    }
  }

  // v0.3.17: 创建 metadata 文件
  const metadata = {
    asset_id: assetId,
    source: "upload",
...
async function handleUploadThumbnail(request, env) {
  ...
  return jsonResponse({
    ...
    thumbnail_url: `/api/thumb/${thumbnailKey}`,
  });
}

// ------------------------------
// GET /api/assets — 素材列表（分页）
// v0.3.17: 全量扫描 Bucket，排除系统目录，检查两种缩略图路径
// ------------------------------
  ...
  for (const obj of pagedImages) {
    ...
    if (assetId) {
      ...
      if (assetThumbnailIndex.has(expectedNewThumbKey)) {
        thumbnailUrl = `/api/thumb/${expectedNewThumbKey}`;
      }
    }
    
    if (!thumbnailUrl) {
      const { key: oldThumbKey } = deriveThumbnailInfo(obj.key, env, thumbnailIndex);
      thumbnailUrl = oldThumbKey ? `/api/thumb/${oldThumbKey}` : "";
    }
 ...
 * v0.3.17: 全量扫描 Bucket，返回 asset_id 用于前端生成缩略图
 */
 ...
 * v0.3.17: 全量扫描 Bucket，同时清理 thumbnails/ 和 assets/thumbnails/ 下的孤儿缩略图
 */
 ...
 * v0.3.17: 全量扫描 Bucket，统计所有图片和两种缩略图
 */
 ...
 * v0.3.17: 全量扫描 Bucket 所有图片（排除系统目录），统一使用 assets/thumbnails/{asset_id}.webp
